...
// Return:            0
//----------------------------------------------------------------------------
MUIDSP ULONG VimConAppMessage( Class *cls,
                               Object *obj,
...
//----------------------------------------------------------------------------
MUIDSP IPTR VimConStopBlink ( Class *cls,
                              Object *obj )
{
...
//----------------------------------------------------------------------------
MUIDSP IPTR VimConIsBlinking ( Class *cls,
                               Object *obj )
{
    struct VimConData *my = INST_DATA (cls,obj);
...
// Return:        0
//----------------------------------------------------------------------------
MUIDSP IPTR VimConAboutMUI ( Class *cls,
                             Object *obj )
{
    struct VimConData *my = INST_DATA (cls,obj);
...
// Return:           0
//----------------------------------------------------------------------------
MUIDSP IPTR VimConMUISettings ( Class *cls,
                                Object *obj )
{
    struct VimConData *my = INST_DATA (cls,obj);
...
// Return:            TRUE on state change, FALSE otherwise
//----------------------------------------------------------------------------
MUIDSP IPTR VimConStartBlink ( Class *cls,
                               Object *obj )
{
    struct VimConData *my = INST_DATA (cls,obj);
...
// Return:             TRUE
//----------------------------------------------------------------------------
MUIDSP IPTR VimConSetBlinking ( Class *cls,
                                Object *obj,
...
//----------------------------------------------------------------------------
MUIDSP IPTR VimConBrowse (Class *cls,
                          Object *obj,
...
// Return:              Buffer area
//----------------------------------------------------------------------------
MUIDSP IPTR VimConGetScreenDim (Class  *cls,
                                Object *obj,
...
//----------------------------------------------------------------------------
MUIDSP IPTR VimConSetTitle (Class *cls,
                            Object *obj,
...
//----------------------------------------------------------------------------
MUIDSP IPTR VimConTicker (Class *cls,
                          Object *obj)
{
    struct VimConData *my = INST_DATA (cls,obj);
...
// Return:         TRUE
//----------------------------------------------------------------------------
MUIDSP IPTR VimConTimeout (Class *cls,
                           Object *obj)
{
    struct VimConData *my = INST_DATA (cls,obj);
...
// Return:            TRUE if state changed, FALSE otherwise
//----------------------------------------------------------------------------
MUIDSP IPTR VimConSetTimeout (Class *cls,
                              Object *obj,
...
// Return:       -
//----------------------------------------------------------------------------
MUIDSP void VimConDirty (Class *cls,
                         Object *obj,
                         int x1,
                         int y1,
                         int x2,
...
// Return:       -
//----------------------------------------------------------------------------
MUIDSP void VimConClean (Class *cls,
                         Object *obj)
{
    struct VimConData *my = INST_DATA (cls,obj);
...
// Return:                  TRUE on success, FALSE otherwise
//----------------------------------------------------------------------------
MUIDSP IPTR VimConDrawHollowCursor (Class *cls,
                                    Object *obj,
...
// Return:                TRUE on success, FALSE otherwise
//----------------------------------------------------------------------------
MUIDSP IPTR VimConDrawPartCursor (Class *cls,
                                  Object *obj,
...
// Return:            TRUE on success, FALSE otherwise
//----------------------------------------------------------------------------
MUIDSP IPTR VimConInvertRect (Class *cls,
                              Object *obj,
...
// Return:           TRUE on success, FALSE otherwise
//----------------------------------------------------------------------------
MUIDSP IPTR VimConFillBlock (Class *cls,
                             Object *obj,
...
// Return:             TRUE if state changed, FALSE otherwise
//----------------------------------------------------------------------------
MUIDSP IPTR VimConDeleteLines (Class *cls,
                               Object *obj,
...
// Return:            TRUE
//----------------------------------------------------------------------------
MUIDSP IPTR VimConSetFgColor (Class *cls,
                              Object *obj,
...
// Return:            TRUE
//----------------------------------------------------------------------------
MUIDSP IPTR VimConSetBgColor (Class *cls,
                              Object *obj,
...
//----------------------------------------------------------------------------
MUIDSP IPTR VimConDrawString (Class *cls,
                              Object *obj,
...
//----------------------------------------------------------------------------
MUIDSP IPTR VimConNew (Class *cls,
                       Object *obj,
                       struct opSet *msg)
{
...
// Return:       See MUI docs
//----------------------------------------------------------------------------
MUIDSP IPTR VimConSetup (Class *cls,
                         Object *obj,
...
// Return:         See BOOPSI docs
//----------------------------------------------------------------------------
MUIDSP IPTR VimConDispose (Class *cls,
                           Object *obj,
                           Msg msg)
{
    struct VimConData *my = INST_DATA (cls,obj);
...
// Return:         See MUI docs
//----------------------------------------------------------------------------
MUIDSP IPTR VimConCleanup (Class *cls,
                           Object *obj,
                           Msg msg)
{
    struct VimConData *my = INST_DATA (cls,obj);
...
// Return:          -
//----------------------------------------------------------------------------
static void VimConAddEvent (Class *cls,
                            Object *obj,
                            ULONG event)
{
    struct VimConData *my = INST_DATA (cls,obj);
...
// Return:          -
//----------------------------------------------------------------------------
static void VimConRemEvent (Class *cls,
                            Object *obj,
                            ULONG event)
{
    struct VimConData *my = INST_DATA (cls,obj);
...
//----------------------------------------------------------------------------
MUIDSP int VimConMouseScrollEvent (Class *cls,
                                   Object *obj,
...
// Return:           TRUE
//----------------------------------------------------------------------------
MUIDSP int VimConMouseMove (Class *cls,
                            Object *obj,
                            struct MUIP_HandleEvent *msg)
{
...
//----------------------------------------------------------------------------
MUIDSP int VimConMouseHandleButton (Class *cls,
                                    Object *obj,
                                    struct MUIP_HandleEvent *msg)
{
    static int drag;
...
// Return:           TRUE
//----------------------------------------------------------------------------
MUIDSP int VimConHandleRaw (Class *cls,
                            Object *obj,
                            struct MUIP_HandleEvent *msg)
{
    static TEXT b[4];
...
// Return:             See MUI docs
//----------------------------------------------------------------------------
MUIDSP IPTR VimConHandleEvent (Class *cls,
                               Object *obj,
                               struct MUIP_HandleEvent *msg)
{
    struct VimConData *my = INST_DATA (cls,obj);
...
// Return:        See MUI docs
//----------------------------------------------------------------------------
MUIDSP IPTR VimConMinMax (Class *cls,
                          Object *obj,
...
// Return:      See MUI docs
//----------------------------------------------------------------------------
MUIDSP IPTR VimConDraw (Class *cls,
                        Object *obj,
...
//----------------------------------------------------------------------------
MUIDSP IPTR VimConCallback (Class *cls,
                            Object *obj,
...
//----------------------------------------------------------------------------
MUIDSP IPTR VimConGetState (Class *cls,
                            Object *obj)
{
    struct VimConData *my = INST_DATA (cls,obj);
...
// Return:      See MUI docs
//----------------------------------------------------------------------------
MUIDSP IPTR VimConShow (Class *cls,
                        Object *obj,
                        Msg msg)
{
...
// Return:      TRUE
//----------------------------------------------------------------------------
MUIDSP IPTR VimConBeep (Class *cls,
                        Object *obj)
{
    struct VimConData *my = INST_DATA (cls,obj);
...
// Return:      0
//----------------------------------------------------------------------------
MUIDSP ULONG VimConCopy( Class *cls,
                         Object *obj,
...
// Return:       0
//----------------------------------------------------------------------------
MUIDSP ULONG VimConPaste( Class *cls,
                          Object *obj,
...
// Return:               TRUE on success, FALSE otherwise
//----------------------------------------------------------------------------
MUIDSP IPTR VimToolbarAddButton (Class *cls,
                                 Object *obj,
                                 struct MUIP_VimToolbar_AddButton *msg)
{
    ...
    {
        const char *n = (const char *) msg->Label,
                   *h = b->help;
...
// Return:                   TRUE on success, FALSE otherwise
//----------------------------------------------------------------------------
MUIDSP IPTR VimToolbarDisableButton (Class *cls,
                                     Object *obj,
                                     struct MUIP_VimToolbar_DisableButton *msg)
{
    ...
    while (b->img != MUIV_TheBar_End)
    {
        if(msg->ID == (ULONG) b->_class)
        {
            ...
            DoMethod (obj, MUIM_TheBar_SetAttr,
                      b->ID, MUIV_TheBar_Attr_Disabled,
                      msg->Grey);
            return TRUE;
        }
        else
...
// Return:         See BOOPSI docs
//----------------------------------------------------------------------------
MUIDSP IPTR VimToolbarNew (Class *cls,
                           Object *obj,
                           struct opSet *msg)
{
...
// Return:       TRUE on success, FALSE otherwise
//----------------------------------------------------------------------------
MUIDSP IPTR VimMenuGrey (Class *cls,
                         Object *obj,
                         struct MUIP_VimMenu_Grey *msg)
{
        ...
        (menu->parent && menu_is_toolbar (menu->parent->name) ) )
    {
        if(Tlb)
        {
            DoMethod (Tlb, MUIM_VimToolbar_DisableButton,
                      menu, msg->Grey);
        }
        return TRUE;
    }
...
// Return:             TRUE on success, FALSE otherwise
//----------------------------------------------------------------------------
MUIDSP IPTR VimMenuRemoveMenu (Class *cls,
                               Object *obj,
...
//----------------------------------------------------------------------------
MUIDSP IPTR VimMenuAddSpacer (Class *cls,
                              Object *obj,
                              struct MUIP_VimMenu_AddSpacer *msg)
{
    ...
    i = MUI_NewObject (MUIC_Menuitem, MUIA_Menuitem_Title,
                       NM_BARLABEL, TAG_END);
...
//----------------------------------------------------------------------------
MUIDSP IPTR VimMenuAddMenu (Class *cls,
                            Object *obj,
                            struct MUIP_VimMenu_AddMenu *msg)
{
    ...
    i = MUI_NewObject (MUIC_Menu, MUIA_Menu_Title, msg->Label,
...
//----------------------------------------------------------------------------
MUIDSP IPTR VimMenuAddMenuItem (Class *cls,
                                Object *obj,
                                struct MUIP_VimMenu_AddMenuItem *msg)
{
    ...
    DoMethod (m, MUIM_Family_AddTail, i);
    DoMethod (i, MUIM_Notify, MUIA_Menuitem_Trigger, MUIV_EveryTime,
              Con, 2, MUIM_VimCon_Callback, msg->ID);
    return (IPTR) i;
}
...
//----------------------------------------------------------------------------
GuiFont gui_mch_get_font (char_u *vim_font_name,
                          int report_error)
{
    INFO ("Not supported");
    return (GuiFont) NULL;
...
//----------------------------------------------------------------------------
char_u *gui_mch_get_fontname (GuiFont font,
                              char_u *name)
{
    INFO ("Not supported");
    return NULL;
...
//----------------------------------------------------------------------------
void gui_mch_enable_scrollbar (scrollbar_T *sb,
                               int flag)
{
    INFO ("Not supported");
}
...
//----------------------------------------------------------------------------
void gui_mch_create_scrollbar (scrollbar_T *sb,
                               int orient)
{
    INFO ("Not supported");
}
...
//----------------------------------------------------------------------------
void gui_mch_set_scrollbar_thumb (scrollbar_T *sb,
                                  int val,
                                  int size,
                                  int max)
{
...
//----------------------------------------------------------------------------
void gui_mch_set_scrollbar_pos (scrollbar_T *sb,
                                int x,
                                int y,
                                int w,
                                int h)
...
// gui_mch_draw_string
//----------------------------------------------------------------------------
void gui_mch_draw_string (int row,
                          int col,
                          char_u *s,
                          int len,
                          int flags)
...
//----------------------------------------------------------------------------
void gui_mch_set_shellsize (int width,
                            int height,
                            int min_width,
                            int min_height,
                            int base_width,
...
// gui_mch_clear_block
//----------------------------------------------------------------------------
void gui_mch_clear_block (int row1,
                          int col1,
                          int row2,
                          int col2)
{
    DoMethod (Con, MUIM_VimCon_FillBlock, row1, col1,
              row2, col2, gui.back_pixel);
}

//----------------------------------------------------------------------------
// gui_mch_delete_lines
//----------------------------------------------------------------------------
void gui_mch_delete_lines (int row,
                           int num_lines)
{
...
//----------------------------------------------------------------------------
void gui_mch_insert_lines (int row,
                           int num_lines)
{
    DoMethod (Con, MUIM_VimCon_DeleteLines, row, -num_lines,
              gui.scroll_region_left, gui.scroll_region_right,
...
{
    DoMethod (Con, MUIM_VimCon_FillBlock, 0, 0, gui.num_rows,
...
//----------------------------------------------------------------------------
void gui_mch_set_menu_pos (int x,
                           int y,
                           int w,
                           int h)
{
...
int gui_mch_init (void)
{
    ...
    DoMethod (Win, MUIM_Notify, MUIA_AppMessage, MUIV_EveryTime,
              Con, 2, MUIM_VimCon_AppMessage, MUIV_TriggerValue);

    // MUI specific menu parts
    if(Abo && Set)
    {
        DoMethod (Abo, MUIM_Notify, MUIA_Menuitem_Trigger, MUIV_EveryTime,
                  Con, 1, MUIM_VimCon_AboutMUI);
        DoMethod (Set, MUIM_Notify, MUIA_Menuitem_Trigger, MUIV_EveryTime,
                  Con, 1, MUIM_VimCon_MUISettings);
    }

    // From where do we get these defaults?
    gui.def_norm_pixel = gui.norm_pixel =
                         gui_mch_get_color ( (char_u *) "Grey");
    gui.def_back_pixel = gui.back_pixel =
                         gui_mch_get_color ( (char_u *) "Black");
...
//----------------------------------------------------------------------------
int gui_mch_init_font (char_u *vim_font_name,
                       int fontset)
{
    return OK;
}
...
{
    DoMethod (Con, MUIM_VimCon_DrawHollowCursor,
              gui.row, gui.col, gui.norm_pixel);
}
...
void gui_mch_draw_part_cursor (int w,
                               ...
                               guicolor_T color)
{
    DoMethod (Con, MUIM_VimCon_DrawPartCursor, gui.row,
              gui.col, w, h, gui.norm_pixel);
}

//----------------------------------------------------------------------------
// gui_mch_set_text_area_pos - Not supported (let MUI handle this)
//----------------------------------------------------------------------------
void gui_mch_set_text_area_pos (int x,
                                int y,
                                int w,
                                int h)
{
...
// gui_mch_add_menu
//----------------------------------------------------------------------------
void gui_mch_add_menu (vimmenu_T *menu,
                       int index)
{
    if (Mnu &&
        !menu_is_popup (menu->name) &&
        !menu_is_toolbar (menu->name))
    {
...
//                         and discarding.
//----------------------------------------------------------------------------
void gui_mch_add_menu_item (vimmenu_T *menu,
                            int index)
{
    ...
    if (Tlb && menu_is_toolbar (p->name) &&
        !menu_is_separator (menu->name))
    {
        DoMethod (Tlb, MUIM_VimToolbar_AddButton, menu,
                  menu->dname, menu->dname);
    }
    else
    {
        if(Mnu)
...
// gui_mch_menu_grey
//----------------------------------------------------------------------------
void gui_mch_menu_grey (vimmenu_T *menu,
                        int grey)
{
    if(Mnu)
    {
...
//----------------------------------------------------------------------------
void gui_mch_menu_hidden (vimmenu_T *menu,
                          int hidden)
{
    INFO ("Not supported");
}
...
//----------------------------------------------------------------------------
void gui_mch_invert_rectangle (int row,
                               int col,
                               int nr,
                               int nc)
{
...
//----------------------------------------------------------------------------
void gui_mch_set_blinking (long wait,
                           long on,
                           long off)
{
...
// gui_mch_settitle
//----------------------------------------------------------------------------
void gui_mch_settitle (char_u *title,
                       char_u *icon)
{
